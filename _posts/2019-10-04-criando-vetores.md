...
Quando uma operação é repetida para um conjunto de valores de entrada, é natural e vantajoso representar os dados como Arrays Multidimensionais(Vetores, Matrizes e tensores) e operações que são feitas sobre arrays são ditas vetorizadas. Operações vetorizadas eliminam a necessidade explícitos sobre os elementos de uma matriz, o resultado é código conciso, mais legível, e permitindo delegar a implementação de operações elemento a elemento a linguagens de baixo nível(C, C++ e Fortran) que são mais eficientes.
...
- Arrays Numpy são tipadas, ou seja, na sua criação é definido tipo dos dados armazenados.
...
- São de tamanho fixo, significa que um Array Numpy não pode redimensionado, a menos que você crie um novo array.
...
No exemplo acima criamos um array usando a função *np.array*, em posts futuros veremos outras formas de criar um array, depois de criado, acessamos alguns atributos, os dados estão dispostos em um array de duas dimensões(dados.ndim) de shape 3x2 apresentado com dados.shape, e possui um total de seis elementos(dados.size) do tipo int64(dados.dtype) o que equivale a um tamanho total de 48 bytes(dados.nbytes)
